...
class DistSAGE(nn.Module):
    ...
    def full_graph_inference(self, graph, features):
        h = features
...
def sar_evaluate(fgm, g, model, device):
    """
    Evaluate the model on the validation and test sets with SAR.
    fgm : SAR's GraphShardManager to perform full graph inference
    g : DistGraph - DistDGL graph
    model : Trained model on which perform inference run 
    val_nid : the node Ids for validation.
    batch_size : Number of nodes to compute at the same time.
    device : The device to evaluate on
    """
    # Obtain validation and test masks
    # These are stored in local_partition of DistGraph as DistTensor
    # Map to original NID in case of node reordering in DistDGL
    local_part = g.local_partition
    orig_n_ids = local_part.ndata[dgl.NID][local_part.ndata['inner_node'].bool().nonzero().view(-1)]

    features = g.ndata['features'][orig_n_ids]
    labels = g.ndata['labels'][orig_n_ids]
    test_mask = g.ndata['test_mask'][orig_n_ids].nonzero(
            as_tuple=False).view(-1)
    val_mask = g.ndata['val_mask'][orig_n_ids].nonzero(
            as_tuple=False).view(-1)

    model.eval()
    with th.no_grad():
        logits = model.full_graph_inference(fgm, features)
    model.train()
    ...
    acc_vec = th.FloatTensor(results)
    # Sum the n_correct, and number of mask elements across all workers
    sar.comm.all_reduce(acc_vec, op=th.distributed.ReduceOp.SUM, move_to_comm_device=True)
    (val_acc, test_acc) =  \
            (acc_vec[0] / acc_vec[1],
             acc_vec[2] / acc_vec[3])
...
def run(args, device, data):
    ...
    full_graph_manager = sar.convert_dist_graph(g).to(device)
    shuffle = True
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0 and epoch != 0:
            ...
            print(
                "DistDGL: Part {}, Val Acc {:.4f}, Test Acc {:.4f}, time: {:.4f}".format
                (
            ...
            # SAR Evaluation
            start = time.time()
            val_acc, test_acc = sar_evaluate(full_graph_manager, g, model.module, device)
            print(
                "SAR: Part {}, Val Acc {:.4f}, Test Acc {:.4f}, time: {:.4f}".format
                (
                    g.rank(), val_acc, test_acc, time.time() - start
                )
            )
...
def main(args):
    ...
    sar.initialize_comms(g.rank(),
                         g.get_partition_book().num_partitions(),
                         master_ip_address,
                         args.backend
                         )
